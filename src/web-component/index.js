...
class PdbeMolstar extends LitElement {
  ...
  formatParam(propName) {
    ...
    if (propName === "customDataUrl" || propName === "customDataFormat") {
      ...
      ligandParams.forEach((ligandParam) => {
        if (this[ligandParam]) {
          this.initParams.ligandView[ligandParamsMap[ligandParam]] =
            this[ligandParam];
        }
      });
    } else if (
      ["highlightColorR", "highlightColorG", "highlightColorB"].indexOf(
        propName
      ) > -1 &&
      this.highlightColorR &&
      this.highlightColorG &&
      this.highlightColorB
    ) {
      ...
      ["selectColorR", "selectColorG", "selectColorB"].indexOf(propName) > -1 &&
      this.selectColorR &&
      this.selectColorG &&
      this.selectColorB
    ) {
      ...
      ["bgColorR", "bgColorG", "bgColorB"].indexOf(propName) > -1 &&
      this.bgColorR &&
      this.bgColorG &&
      this.bgColorB
    ) {
      ...
      hideStructParams.forEach((hideStructParam) => {
        if (this[hideStructParam]) {
          this.initParams.hideStructure.push(
            hideStructParamsMap[hideStructParam]
          );
        }
      });
      ...
      hideIconsParams.forEach((hideIconsParam) => {
        if (this[hideIconsParam]) {
          this.initParams.hideCanvasControls.push(
            hideIconsParamsMap[hideIconsParam]
          );
        }
      });
    }
  ...
  connectedCallback() {
    ...
    const paramValidatity = this.validateParams();
