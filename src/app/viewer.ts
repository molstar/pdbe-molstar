...
import * as InterfaceOpening from './extensions/interface-opening';
...
export class PDBeMolstarPlugin {
    ...
    visual = {
        ...
        setViewDirection: async (view: 'front' | 'back' | 'right' | 'left' | 'top' | 'bottom' | Mat3, options?: { durationMs: number }) => {
            ...
            const rot = getRotationMat4(view);
            ...
            const newDir = Vec3.create(0, 0, -1);
            Vec3.transformMat4(newDir, newDir, rot);
            Vec3.scale(newDir, newDir, Vec3.distance(snapshot.position!, snapshot.target!));
